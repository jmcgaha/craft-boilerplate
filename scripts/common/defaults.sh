...
# Staging path constants; paths should always have a trailing /
STAGING_ROOT_PATH="REPLACE_ME"
STAGING_ASSETS_PATH=${STAGING_ROOT_PATH}"REPLACE_ME"

# Should we connect to the staging database server via ssh?
STAGING_DB_USING_SSH="yes"

# Staging database constants; default port for mysql is 3306, default port for postgres is 5432
...
# If you are using mysql 5.6.10 or later and you have `login-path` setup as per:
# https://opensourcedbms.com/dbms/passwordless-authentication-using-mysql_config_editor-with-mysql-5-6/
# you can use it instead of the above STAGING_DB_* constants; otherwise leave this blank
STAGING_DB_LOGIN_PATH=""

# The `mysql` and `mysqldump` commands to run remotely
STAGING_MYSQL_CMD="mysql"
STAGING_MYSQLDUMP_CMD="mysqldump"

# The `psql` and `pg_dump` commands to run remotely
STAGING_PSQL_CMD="psql"
STAGING_PG_DUMP_CMD="pg_dump"

# Staging backups path; paths should always have a trailing /
STAGING_BACKUPS_PATH="/tmp/"

# Staging Amazon S3 bucket name
STAGING_S3_BUCKET="REPLACE_ME"
